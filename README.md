...
1. **Setting Up the Modal Presentation Segue**
   ...
   - To enable editing of existing emojis, also control-drag from the **prototype cell** to the **Navigation Controller** of the **Add/Edit Emoji Table View Controller**, using `"EditEmoji"` as the identifier and **Present Modally** as the segue type.
...
3. **Implementing the Unwind Segue Action in EmojiTableViewController**
...
4. **Creating a Custom Class for Add/Edit Emoji**
   ...
   - Declare the `var emoji : Emoji?` property in the `AddEditEmojiTableViewController` class.

5. **Setting Up the Table View Style and Layout for Add/Edit Screen**
   ...
   - In **Sections**, set the number of sections to **4** (one for each property of the emoji).

6. **Adding Titles and Text Fields for Each Section**
...
7. **Connecting Outlets and Configuring Save Logic**
   ...
   - Connect all the text fields to the `textEditingChanged` `@IBAction` method.

8. **Adapting AddEditEmojiTableViewController to the adding and editing modes**
...
9. **Implementing the `prepare(for:)` Method in AddEditEmojiTableViewController**

   - Implement `prepare(for:)` in `AddEditEmojiTableViewController` to save the emoji with the current field values:
     ```swift
     override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
...
10. **Setting Up the Unwind Segue in the Storyboard**
